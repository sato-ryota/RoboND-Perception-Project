...
# Exercise-2 TODOs:
    ...
    stat_cloud = cloud.make_statistical_outlier_filter()

    # Set the number of neighboring points to analyze for any given point
    stat_cloud.set_mean_k(50)

    # Set threshold scale factor
    x = 0.01

    # Any point with a mean distance larger than global (mean distance+x*std_dev) will be considered outlier
    stat_cloud.set_std_dev_mul_thresh(x)

    # Finally call the filter function for magic
    cloud = stat_cloud.filter()
    ...
    # separating the objects and table from others(boxes and side tables)
    passthrough = cloud_filtered.make_passthrough_filter()
    filter_axis = 'y'
    passthrough.set_filter_field_name(filter_axis)
    axis_min = -0.43
    ...
    # Checking the efficency of the filter
...
#    detected_objects = []
    ...
    for index, pts_list in enumerate(cluster_indices):
        ...
        # Compute the ass ociated feature vector
...
#aa        rospy.wait_for_service('pick_place_routine')

#aa        try:
#aa            pick_place_routine = rospy.ServiceProxy('pick_place_routine', PickPlace)

            # TODO: Insert your message variables to be sent as a service request
#            resp = pick_place_routine(TEST_SCENE_NUM, OBJECT_NAME, WHICH_ARM, PICK_POSE, PLACE_POSE)
#aa            resp = pick_place_routine(test_scene_num, object_name, arm_name, pick_pose, place_pose)

#aa            print ("Response: ",resp.success)

#aa        except rospy.ServiceException, e:
#aa            print "Service call failed: %s"%e
